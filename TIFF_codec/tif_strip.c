...
{
	uint32 nstrips;	TIFFDirectory *td = &tif->tif_dir;
	...
	if (td->td_planarconfig == PLANARCONFIG_SEPARATE)
		/*nstrips = _TIFFMultiply32(tif, nstrips, (uint32)td->td_samplesperpixel,
		    "TIFFNumberOfStrips");*/
		nstrips = nstrips * (uint32)td->td_samplesperpixel; //my patch
	return (nstrips);
}
...
TIFFScanlineSize64(TIFF* tif)
{
	...
	/*if (td->td_planarconfig==PLANARCONFIG_CONTIG)
	{
		...
		scanline_size=TIFFhowmany_64(td->td_imagewidth * td->td_bitspersample, 8);
