...
#define LOOKUP8(wid,tab,eoflab) do {			\ //wid= 7
    NeedBits8(wid,eoflab);						\
    TabEnt = tab + GetBits(wid);				\
    ClrBits(TabEnt->Width);						\
} while (0)

#define LOOKUP16(wid,tab,eoflab) do {			\ //wid=12(white), 13(black)
...
struct proto *MainTables[7] = {Pass, Horiz, V0, VR, VL, Ext, EOLH};    
TIFFFaxTabEnt	*GetSingleFaxMainTabEntry(uint8 ImgByte)
{
...
TIFFFaxTabEnt	*GetSingleFaxWhiteTabEntry(uint16 ImgWord12Bit)
{
	int NumOfTable;
	int limit = 1 << 13;	
	...
	{
		struct proto *P = WhiteTables[NumOfTable];
    	while (P->val) 
		{
			...
			{
				if (code==ImgWord12Bit)
				{
	    			E->State = WhiteTableStates[NumOfTable];
	    			E->Width = width;
	    			E->Param = param;
					goto return_result;
				}
...
TIFFFaxTabEnt	*GetSingleFaxWhiteTabEntryMath(uint16 ImgWord12Bit)
{
	int NumOfTable;
	int limit = 1 << 13;	
	TIFFFaxTabEnt *E = &SingleFaxTabEntry;	
...
TIFFFaxTabEnt	*GetSingleFaxBlackTabEntry(uint16 ImgWord13Bit)
{
	int NumOfTable;
	int limit = 1 << 13;	
	TIFFFaxTabEnt *E = &SingleFaxTabEntry;	
	...
	for(NumOfTable=0; NumOfTable<4; NumOfTable++)
	{
		struct proto *P = BlackTables[NumOfTable];
    	while (P->val) 
		{
			int width = P->val & 0x0F;
			int param = P->val >> 4;
			...
			for (code = P->code; code < limit; code += incr) 
			{
				if (code==ImgWord13Bit)
				{
	    			E->State = BlackTableStates[NumOfTable];
	    			E->Width = width;
	    			E->Param = param;
					goto return_result;
				}
...
/*    fprintf(fd, "#include \"tiff.h\"\n");
    ...
    for(i=0;i<4096;i++)
    {
    	pTabEnt=GetSingleFaxWhiteTabEntryMath(i);
    	if (pTabEnt==NULL)
		{
    		printf("Error! i=%d\n", i);
    		return -1;
    	}
    	/*if((pTabEnt->State != WhiteTable[i].State) ||
    		(pTabEnt->Param != WhiteTable[i].Param) ||
    		(pTabEnt->State != WhiteTable[i].State))
    		printf("Tables mismatch! i=%d\n", i);*/
	}	
    
    for(i=0;i<8192;i++)
    {
    	pTabEnt=GetSingleFaxBlackTabEntry(i);
    	if (pTabEnt==NULL)
		{
    		printf("Error! i=%d\n", i);
    		return -1;
    	}
    	/*if((pTabEnt->State != BlackTable[i].State) ||
    		(pTabEnt->Param != BlackTable[i].Param) ||
    		(pTabEnt->State != BlackTable[i].State))
    		printf("Tables mismatch! i=%d\n", i);*/
	}	
    return (0);
}
